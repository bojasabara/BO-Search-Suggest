...
class BOSS_SearchClass {
    private $options;

    public function __construct(){
        $content_url = wp_upload_dir();
        $this->options = get_option( 'bo_ss_searchsuggest_plugin_options' );
    ...
    public function search($bo_ss_search){
        global $wpdb;

        $results = get_transient('bo_ss_'.$bo_ss_search);
        /* IF NOT CACHED */
        if(!$results){

            //Remove Stop words
            if(isset($this->options['stop_words']) && !empty($this->options['stop_words'])){
                $bo_ss_search = $this->remove_stop_words($bo_ss_search);
            }

            $prepare =  array($this->options['show_cat'], $this->options['show_thumbnail'], $this->options['show_price'], '%' . $wpdb->esc_like($bo_ss_search) .'%');
            $join = '';
            $where = '';
            ...
            if(isset($this->options['taxonomies_search']) && !empty($this->options['taxonomies_search']) && is_array($this->options['taxonomies_search'])){
                $results_cat = $this->get_taxonomies_search($bo_ss_search);
            }
            ...
            $this->results = apply_filters('bo_ss_autosuggest_results', $this->results);
            //Save data in transient for 24h
            
            $transient_to_delete = get_transient('bo_ss_transient');
            if($transient_to_delete){
                $transient_to_delete[] = $bo_ss_search;
                set_transient('bo_ss_transient', $transient_to_delete);
            }
            else{
                $transient_to_delete = array($bo_ss_search);
                set_transient('bo_ss_transient', $transient_to_delete);
            }

            set_transient('bo_ss_'.$bo_ss_search, $this->results, 86400);
    ...
    private function get_taxonomies_search($bo_ss_search){
        global $wpdb;
        $prepare = $this->options['taxonomies_search'];
        $prepare[] = '%' . $wpdb->esc_like($bo_ss_search) .'%';
